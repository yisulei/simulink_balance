...
void timer_5ms_callback(void)
{
  ...
  if (time_5ms_count%2==0) {
    ...
    UpdateOneColorToAllBit(ws2812_data,pwm4_1_DMAData,WS2812_NUMS);
    HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1,(uint32_t*)pwm4_1_DMAData,
                          WS2812_NUMS*24+1);
                                      /* start DMA transfer on TIM4 channel 1 */
    HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_2,(uint32_t*)pwm4_2_DMAData,
                          WS2812_NUMS*24+1);
                                      /* start DMA transfer on TIM4 channel 2 */
  }
...
int main(void)
{
  ...
  HAL_UART_Receive_DMA(&huart1, Uart1DMAData.rxDMABuf, UART1DMA_RXBUFFER_SIZE);//set Buffer,??rtIOStreamOpen????
  ...
  __HAL_TIM_SetCompare(&htim4,TIM_CHANNEL_1,0);//6->0;13->1
